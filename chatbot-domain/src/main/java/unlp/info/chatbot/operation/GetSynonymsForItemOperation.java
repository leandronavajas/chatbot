...
@Component
public class GetSynonymsForItemOperation implements Operation<GetExpressionsForItemRequest, List<Entity>> {
  ...
  public List<Entity> execute(GetExpressionsForItemRequest request) {
    List<EntityPersistent> synonyms = this.messageRepositoryService.getSynonyms(request.getCategoryId(), request.getItemId());

    return this.entityListTransformer.transform(synonyms);
  }
